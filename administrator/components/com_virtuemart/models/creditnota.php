 ...
 * @subpackage Coupon
 */
class VirtueMartModelCreditnota extends VmModel {
	...
	function __construct() {
		parent::__construct();
		$this->setMainTable('creditnota');
	}

	static public function isCreditnotaToBeAttachByOrderstats($credit_status){

		return (int)(VirtueMartModelCreditnota::needInvoiceByOrderstatus($order_status) or VirtueMartModelInvoice::needInvoiceByOrderstatus($order_status,'inv_osr', array('R')));
	}

	static function needInvoiceByOrderstatus($credit_status, $confName = 'cre_s', $def = array('C')){

		$creditnotastatusForCreditnota = VmConfig::get($confName,$def);
		if(!is_array($creditnotastatusForCreditnota)) $creditnotastatusForCreditnota = array($creditnotastatusForCreditnota);

		$creditnotaOrderStatus = false;
		if(in_array($order_status, $creditnotastatusForCreditnota)){
			$creditnotaOrderStatus = true;
		}

		return $creditnotaOrderStatus;
	}
